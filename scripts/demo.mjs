...
async function main() {
  ...
  function write(path, text) { writeFileSync(join(repo, path), text); }
  ...
  write("tsconfig.json", JSON.stringify({ compilerOptions: { module: "esnext", moduleResolution: "bundler", strict: true }, include: ["src"] }));
  write("src/pricing.ts", "export function total(unitPrice: number, quantity: number): number {\n  return unitPrice * quantity;\n}\n");
  write("src/orders.ts", 'import { total } from "./pricing.js";\nexport function submitOrder(unitPrice: number): number {\n  return total(unitPrice, 1);\n}\n');
  write("src/pricing.test.ts", 'import { total } from "./pricing.js";\nexport function testTotal(): boolean {\n  return total(10, 2) === 20;\n}\n');
  git("add", ".");
  ...
  const env = { ...process.env, GRAPH_PROVIDER: "scip", SCIP_REPO_ROOT: repo,
    CRW_DATA_DIR: join(workspace, "data"), CRW_INDEXER_HOME: join(project, "packages/server"),
    CRW_SERVER_URL: `http://127.0.0.1:${port}` };
  function crw(...args) {
    return JSON.parse(execFileSync(process.execPath, [cli, ...args], { cwd: repo, env, encoding: "utf8", stdio: ["ignore", "pipe", "inherit"] }));
  }
  let running = false;
  try {
    ...
    writeFileSync(planPath, JSON.stringify({
      overview: "Orders now accept a quantity, with a 10% discount at ten items. Follow submitOrder into pricing, then inspect the attached tests. Consider which inputs the new quantity parameter permits.",
      units: context.flows.map((flow) => ({ kind: "flow", flowIds: [flow.id], label: "Order quantities and bulk pricing", rationale: "The order entry passes a new quantity through to the discount calculation." })),
    }));
    crw("plan", "--session", session.sessionId, "--units", planPath);
    const quotedCli = "'" + cli.replaceAll("'", "'\\''") + "'";
    console.log(`Example review: ${session.uiUrl}\n\nTry j to start, r to mark reviewed, and c to leave a comment.\nDoes the new quantity need validation? Use Review notes for broader concerns.\n\nTemporary example: ${workspace}\nStop its hub: node ${quotedCli} shutdown --port ${port}`);
  } catch (error) {
    if (running) { try { crw("shutdown"); } catch { /* preserve original error */ } }
    console.error(`Demo failed: ${error.message}\nTemporary files: ${workspace}`);
    process.exitCode = 1;
  }
}
main().catch((error) => { console.error(`Demo setup failed: ${error.message}`); process.exitCode = 1; });
