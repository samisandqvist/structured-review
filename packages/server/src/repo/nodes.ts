...
export function getNodeNeighbors(db: DB, nodeId: string): { callers: Node[]; callees: Node[] } {
  const callers = (
    db
      .prepare(`SELECT n.* FROM nodes n JOIN edges e ON e.source_node_id = n.id WHERE e.target_node_id = ?`)
      .all(nodeId) as NodeRow[]
  ).map(rowToNode);
  const callees = (
    db
      .prepare(`SELECT n.* FROM nodes n JOIN edges e ON e.target_node_id = n.id WHERE e.source_node_id = ?`)
      .all(nodeId) as NodeRow[]
  ).map(rowToNode);
...
export function updateNodeReviewStatus(db: DB, id: string, status: ReviewStatus, reviewedInUnit?: number): void {
  db.prepare("UPDATE nodes SET review_status = ?, reviewed_in_unit = ? WHERE id = ?").run(
    status,
    reviewedInUnit ?? null,
    id,
  );
