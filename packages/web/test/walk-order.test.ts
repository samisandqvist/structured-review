...
const node = (id: string, review = "unreviewed"): Node => ({
  ...
  reviewStatus: review as Node["reviewStatus"],
  reviewedInUnit: null,
  isTest: false,
});
const step = (id: string, changed = true): FlowStep => ({
  ...
  isTest: false,
  depth: 0,
  nodeId: changed ? `n-${id}` : null,
  changeStatus: changed ? "changed" : null,
  reviewStatus: null,
});
const unit = (
  id: string,
...
describe("buildWalkOrder", () => {
  ...
  it("walks counted attachments right after their parent; references never walk", () => {
    ...
    const units = [
      unit("u1", "flow", ["e"], 0, [{ stableId: "t", parentStableId: "e", reason: "tested-by", counted: true }]),
    ...
    expect(buildWalkOrder(units, flows, nodes).map((w) => w.stableId)).toEqual(["e", "t", "next", "orphan1", "dto"]);
  });
});
