...
function node(stableId: string, over: Partial<SessionNode> = {}): SessionNode {
  return {
    id: `id-${stableId}`, stableId, label: stableId, file: "f.ts",
    startLine: 1, endLine: 10, changeStatus: "changed", reviewStatus: "unreviewed", isTest: false,
    ...over,
  };
}
...
function flow(entry: string, stepIds: string[]): FlowDTO {
  return {
    id: 1, name: entry, affected: true, entryStableId: entry, changedStableIds: [],
    steps: stepIds.map((s, i) => ({
      stableId: s, label: s, file: "f.ts", startLine: 1, endLine: 10,
      isTest: false, depth: i, nodeId: `id-${s}`, changeStatus: null, reviewStatus: null,
    })),
  };
}
...
describe("waitConditionMet", () => {
  const base = {
    sessionId: "s1", sessionStatus: "walking",
    coverage: { changedTotal: 2, covered: 2, unassigned: 0 }, units: [],
  };
  it("'reviewed' is met when no changed node is unreviewed", () => {
    expect(waitConditionMet("reviewed", { ...base, unreviewed: [] }, 0)).toBe(true);
    expect(waitConditionMet("reviewed", { ...base, unreviewed: [{ stableId: "x", label: "x", file: "f" }] }, 5)).toBe(false);
  });
  it("'commented' is met when at least one comment exists", () => {
    expect(waitConditionMet("commented", { ...base, unreviewed: [] }, 0)).toBe(false);
    expect(waitConditionMet("commented", { ...base, unreviewed: [{ stableId: "x", label: "x", file: "f" }] }, 1)).toBe(true);
  });
});
